...
results_root="${RESULTS_ROOT:-${repo_dir}/results/005_storage_kernels}"
build_jobs="${BUILD_JOBS:-4}"
cuda_arch="${CUDA_ARCH:-90}"
dot_count="${DOT_COUNT:-262147}"
gemv_rows="${GEMV_ROWS:-257}"
gemv_columns="${GEMV_COLUMNS:-1027}"
...
{
    echo "experiment=005_storage_kernels"
...

{
    ...
    echo "gemv_columns=${gemv_columns}"
...
validator="${build_dir}/bin/storage_kernels_cuda_test"
"${validator}" \
    --dot-count "${dot_count}" \
    --gemv-rows "${gemv_rows}" \
    --gemv-columns "${gemv_columns}" \
    --output-dir "${run_dir}" \
...
echo
echo "Storage-kernel validation complete:"
echo "  ${run_dir}"
echo "  ${run_dir}/dot_validation.csv"
echo "  ${run_dir}/gemv_validation.csv"
