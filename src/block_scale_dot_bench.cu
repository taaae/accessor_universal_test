...
data prepare(std::size_t n,int fixture=0){const bool edge=fixture==1,positive=fixture==2;data d;d.n=n;d.fl.reset(n);d.fr.reset(n);d.dl.reset(n);d.dr.reset(n);d.p32.reset(bs::grid_blocks);d.out32.reset(1);d.p64.reset(bs::grid_blocks);d.out64.reset(1);for(int z=0;z<3;++z){auto&x=d.e[z];x.b=bs::block_sizes[z];x.ql.reset(n);x.qr.reset(n);auto nb=bs::ceil_div(n,x.b);x.sl32.reset(nb);x.sr32.reset(nb);x.sl64.reset(nb);x.sr64.reset(nb);}
 constexpr std::size_t chunk=1u<<20;for(std::size_t off=0;off<n;off+=chunk){std::size_t m=std::min(chunk,n-off);std::vector<float>l(m),r(m);std::vector<double>ld(m),rd(m);for(std::size_t j=0;j<m;++j){auto i=off+j;l[j]=positive?.25f:bs::source_value(bs::value_left_seed,i);r[j]=positive?.5f:bs::source_value(bs::value_right_seed,i);if(edge&&i<12){static const float q[]{0.f,1.f,-1.f,std::nextafter(1.f,2.f),std::nextafter(-1.f,-2.f),.5f,-.5f,.25f,-.25f,1.f,-1.f,0.f};l[j]=q[i];r[j]=q[11-i];}ld[j]=l[j];rd[j]=r[j];}copy_at(d.fl.p,off,l);copy_at(d.fr.p,off,r);copy_at(d.dl.p,off,ld);copy_at(d.dr.p,off,rd);
  for(auto&x:d.e){std::vector<float>ql(m),qr(m);for(std::size_t j=0;j<m;++j){auto i=off+j;float sl=edge?(i/x.b%3==0?.50000006f:i/x.b%3==1?1.00000012f:1.99999988f):(positive?1.25f:bs::scale_value(bs::scale_left_seed,i/x.b));float sr=edge?(i/x.b%2==0?1.99999988f:.50000006f):(positive?.75f:bs::scale_value(bs::scale_right_seed,i/x.b));ql[j]=(edge||positive)?l[j]:bs::encode(l[j],sl);qr[j]=(edge||positive)?r[j]:bs::encode(r[j],sr);}copy_at(x.ql.p,off,ql);copy_at(x.qr.p,off,qr);}
 }
 for(auto&x:d.e){auto nb=bs::ceil_div(n,x.b);for(std::size_t off=0;off<nb;off+=chunk){std::size_t m=std::min(chunk,nb-off);std::vector<float>sl(m),sr(m);std::vector<double>dl(m),dr(m);for(std::size_t j=0;j<m;++j){auto b=off+j;sl[j]=edge?(b%3==0?.50000006f:b%3==1?1.00000012f:1.99999988f):(positive?1.25f:bs::scale_value(bs::scale_left_seed,b));sr[j]=edge?(b%2==0?1.99999988f:.50000006f):(positive?.75f:bs::scale_value(bs::scale_right_seed,b));dl[j]=sl[j];dr[j]=sr[j];}copy_at(x.sl32.p,off,sl);copy_at(x.sr32.p,off,sr);copy_at(x.sl64.p,off,dl);copy_at(x.sr64.p,off,dr);}}
 return d;}
struct variant{std::string id,path,scale;int b;};
std::vector<variant> variants(){std::vector<variant>v;for(auto path:{"per_element","deferred"})for(int b:bs::block_sizes)for(auto scale:{"fp32","fp64"})v.push_back({bs::scaled_id(path,b,scale==std::string("fp64")),path,scale,b});v.push_back({"raw_fp32","baseline","none",0});v.push_back({"fp32_to_fp64","baseline","none",0});v.push_back({"raw_fp64","baseline","none",0});return v;}
encoded& enc(data&d,int b){for(auto&x:d.e)if(x.b==b)return x;throw std::runtime_error("bad B");}
#define LAUNCH(B,T,P,FL,FR) if(v.b==B){auto&x=enc(d,B);bs::P##_kernel<B,T><<<bs::grid_blocks,bs::cta_threads>>>(x.ql.p,x.qr.p,x.FL.p,x.FR.p,d.n,d.p64.p);}
void launch_first(data&d,const variant&v){if(v.path=="baseline"){if(v.id=="raw_fp32")bs::raw32_kernel<<<bs::grid_blocks,bs::cta_threads>>>(d.fl.p,d.fr.p,d.n,d.p32.p);else if(v.id=="fp32_to_fp64")bs::widened32_kernel<<<bs::grid_blocks,bs::cta_threads>>>(d.fl.p,d.fr.p,d.n,d.p64.p);else if(v.id=="raw_fp64")bs::raw64_kernel<<<bs::grid_blocks,bs::cta_threads>>>(d.dl.p,d.dr.p,d.n,d.p64.p);else throw std::runtime_error("unknown baseline");return;}if(v.path=="per_element"){if(v.scale=="fp32"){LAUNCH(16,float,per_element,sl32,sr32)else LAUNCH(32,float,per_element,sl32,sr32)else LAUNCH(128,float,per_element,sl32,sr32)}else{LAUNCH(16,double,per_element,sl64,sr64)else LAUNCH(32,double,per_element,sl64,sr64)else LAUNCH(128,double,per_element,sl64,sr64)}}else if(v.path=="deferred"){if(v.scale=="fp32"){LAUNCH(16,float,deferred,sl32,sr32)else LAUNCH(32,float,deferred,sl32,sr32)else LAUNCH(128,float,deferred,sl32,sr32)}else{LAUNCH(16,double,deferred,sl64,sr64)else LAUNCH(32,double,deferred,sl64,sr64)else LAUNCH(128,double,deferred,sl64,sr64)}}else throw std::runtime_error("unknown path");}
#undef LAUNCH
...
void validate_dataset(std::size_t n,int fixture,std::ofstream&checks,long double&maxerr,long double&maxbound,int&count){const bool edge=fixture==1,positive=fixture==2;auto d=prepare(n,fixture);auto vs=variants();std::map<std::tuple<std::string,int>,double> width;
 for(const auto&v:vs){launch(d,v);CK(cudaDeviceSynchronize());double got=result(d,v);if(!std::isfinite(got))throw std::runtime_error("nonfinite validation result");long double want=0,sa=0;
  ...
  else{auto h=bs::make_arrays(n,v.b);if(edge||positive){for(std::size_t i=0;i<n;++i){static const float q[]{0.f,1.f,-1.f,std::nextafter(1.f,2.f),std::nextafter(-1.f,-2.f),.5f,-.5f,.25f,-.25f,1.f,-1.f,0.f};h.ql[i]=positive?.25f:(i<12?q[i]:bs::source_value(bs::value_left_seed,i));h.qr[i]=positive?.5f:(i<12?q[11-i]:bs::source_value(bs::value_right_seed,i));auto b=i/v.b;h.sl32[b]=positive?1.25f:(b%3==0?.50000006f:b%3==1?1.00000012f:1.99999988f);h.sr32[b]=positive?.75f:(b%2==0?1.99999988f:.50000006f);h.sl64[b]=h.sl32[b];h.sr64[b]=h.sr32[b];}}
   if(v.path=="per_element"){want=bs::reference_per_element(h,v.b);for(std::size_t i=0;i<n;++i)sa+=std::abs((static_cast<long double>(h.ql[i])*h.sl32[i/v.b])*(static_cast<long double>(h.qr[i])*h.sr32[i/v.b]));}else{want=bs::reference_deferred(h,v.b);for(std::size_t base=0;base<n;base+=v.b){long double sub=0;for(std::size_t i=base;i<std::min(n,base+std::size_t(v.b));++i)sub+=static_cast<long double>(h.ql[i])*h.qr[i];sa+=std::abs((static_cast<long double>(h.sl32[base/v.b])*h.sr32[base/v.b])*sub);}}}
  long double eps=v.id=="raw_fp32"?std::numeric_limits<float>::epsilon():std::numeric_limits<double>::epsilon();std::size_t depth=(n+std::size_t(bs::grid_blocks*bs::cta_threads)-1)/std::size_t(bs::grid_blocks*bs::cta_threads)+24;if(v.path=="deferred")depth+=(v.b==16?4:5);long double gamma=(depth*eps)/(1-depth*eps);long double bound=8*gamma*sa+8*eps;long double err=std::abs((long double)got-want);if(err>bound)throw std::runtime_error("CPU/GPU validation mismatch "+v.id);maxerr=std::max(maxerr,err);maxbound=std::max(maxbound,bound);if(v.path!="baseline"){auto key=std::make_tuple(v.path,v.b);if(v.scale=="fp32")width[key]=got;else if(bits(width.at(key))!=bits(got))throw std::runtime_error("scale-width result mismatch");}++count;
 }
 checks<<"validated_n="<<n<<",fixture="<<(edge?"edge":positive?"all_positive":"generated")<<",cases="<<vs.size()<<"\n";
}
}
...
int main(int ac,char**av)try{auto o=parse(ac,av);parent(o.checks);std::ofstream checks(o.checks);long double maxerr=0,maxbound=0;int validated=0;for(std::size_t n:{0u,1u,15u,16u,17u,31u,32u,33u,127u,128u,129u,257u,4099u,1u<<20})validate_dataset(n,0,checks,maxerr,maxbound,validated);validate_dataset(4099,1,checks,maxerr,maxbound,validated);validate_dataset(4099,2,checks,maxerr,maxbound,validated);checks<<std::setprecision(18)<<"max_reference_error="<<maxerr<<"\nmax_reference_bound="<<maxbound<<"\nvalidated_case_count="<<validated<<"\nall_passed=1\n";checks.close();if(o.mode=="validate")return 0;
 ...
 std::vector<std::size_t> sizes=o.mode=="smoke"?std::vector<std::size_t>{1u<<14,1u<<20}:std::vector<std::size_t>(bs::full_sizes.begin(),bs::full_sizes.end());auto vs=variants();parent(o.output);std::ofstream csv(o.output);csv<<"mode,stage,N,B,scale_type,path,variant_id,payload_type,arithmetic,grid,threads,round,order,time_ms,result,valid,job_id,node,seed_set_id,source_commit,binary_sha,sass_sha,audit_sha\n";const char*job=std::getenv("SLURM_JOB_ID"),*node=std::getenv("HOSTNAME");std::mt19937 rng(0x0335eedu);auto started=std::chrono::steady_clock::now();std::size_t done=0,total=sizes.size()*vs.size()*o.samples;
