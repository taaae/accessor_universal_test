...
def validate_contract(
    ...
    raw_rows = [row for row in rows if row["variant"] == "raw_fp64"]
    if len(raw_rows) != CANONICAL_SAMPLES:
        raise ValueError("raw FP64 baseline has the wrong sample count")
    for index, row in enumerate(raw_rows, start=1):
        expected = {
            "mode": "full", "kernel": "dot", "N": str(CANONICAL_N),
            "blocks": "512", "threads": "256", "storage_bits": "64",
            "arithmetic_type": "fp64", "table_location": "none",
            "segments": "0", "coefficient_bytes": "0",
            "warmup": str(CANONICAL_WARMUP), "distribution": "raw",
        }
        for field, value in expected.items():
            if row[field] != value:
                raise ValueError(f"raw row {index}: {field} mismatch")
        finite_positive(row, "kernel_ms", f"raw row {index}")
        if not math.isfinite(float(row["result"])):
            raise ValueError(f"raw row {index}: nonfinite result")
    if {int(row["sample"]) for row in raw_rows} != set(range(CANONICAL_SAMPLES)):
        raise ValueError("raw FP64 baseline has missing or duplicate samples")
    raw_phases = {phase: [row for row in raw_rows if row["phase"] == phase]
                  for phase in ("before", "after")}
    if any(len(group) != CANONICAL_SAMPLES // 2 for group in raw_phases.values()):
        raise ValueError("raw FP64 baseline was not split before and after")
    if any({row["warmups_this_batch"] for row in group} != {"5"}
           for group in raw_phases.values()):
        raise ValueError("raw batches do not have five warmups each")
    if {int(row["execution_order"]) for row in raw_phases["before"]} != {0}:
        raise ValueError("raw before batch has the wrong execution order")
    if {int(row["execution_order"]) for row in raw_phases["after"]} != {21}:
        raise ValueError("raw after batch has the wrong execution order")
    ...
    unknown = {row["variant"] for row in rows} - {"raw_fp64", "dyadic_normal32"}
    if unknown:
    ...
    for target_index, target in enumerate(TARGETS):
        ...
        if {int(row["execution_order"]) for row in phases["forward"]} != {1 + target_index}:
            raise ValueError(f"X={target}: forward execution order is wrong")
        if {int(row["execution_order"]) for row in phases["reverse"]} != {20 - target_index}:
    ...
    if {int(row["execution_order"]) for row in genuine_phases["forward"]} != {10}:
        raise ValueError("genuine forward batch has the wrong execution order")
    if {int(row["execution_order"]) for row in genuine_phases["reverse"]} != {11}:
...
def build_summary(
    ...
    raw = summarize([row for row in rows if row["variant"] == "raw_fp64"])
    genuine_rows = [
        row
        for row in rows
...
def make_svg(
    ...
    lows = [float(row["q1_ms"]) for row in summary] + [float(raw["q1_ms"]), float(genuine["q1_ms"])]
    highs = [float(row["q3_ms"]) for row in summary] + [float(raw["q3_ms"]), float(genuine["q3_ms"])]
    ...
    pieces = [
        ...
        '<desc id="desc">Median DOT kernel time with interquartile ranges, raw FP64 baseline, and expected dispersion for genuine standard-normal inputs.</desc>',
        ...
        f'<text class="subtitle" x="{left}" y="59">DOT, N=2²⁶, scalar x1, FP64 arithmetic; forward/reverse timing batches with raw controls before and after</text>',
    ]
    ...
    raw_y = py(float(raw["median_ms"]))
    raw_q1 = py(float(raw["q1_ms"]))
    raw_q3 = py(float(raw["q3_ms"]))
    pieces.extend(
        [
            f'<rect x="{left}" y="{min(raw_q1, raw_q3):.2f}" width="{plot_width}" height="{max(abs(raw_q3 - raw_q1), 1.0):.2f}" fill="#111820" opacity="0.08"/>',
    ...
    labels = [
        ("DyadicNormal32", "#b5532b", dyadic_y),
        ("Raw FP64", "#111820", raw_y),
    ]
    labels.sort(key=lambda item: item[2])
    if abs(labels[1][2] - labels[0][2]) < 34:
        labels[1] = (labels[1][0], labels[1][1], labels[0][2] + 34)
    for label, color, label_y in labels:
        source_y = dyadic_y if label == "DyadicNormal32" else raw_y
        dash = "3 4" if label == "DyadicNormal32" else "6 5"
    ...
    pieces.append(f'<text class="note" x="{left}" y="{height - 9}">Curve points and the genuine-source diamond each summarize 50 launches. Bars show IQR; raw FP64 was measured before and after.</text>')
    pieces.append("</svg>")
    return "".join(pieces)
...
<p class="lead">This run isolates whether a 512-byte coefficient decoder changes speed when warps spread their accesses across more dyadic segments. Raw FP64 uses the same DOT geometry, executable, GPU allocation, and timing method. A separately generated genuine-standard-normal point checks the bank-access pattern that the one-dimensional X metric cannot capture.</p>
<div class="cards"><div class="card"><b>Input</b><code>N=2^26</code>, scalar x1 DOT</div><div class="card"><b>Geometry</b>512 × 256 first stage, then one reduction block</div><div class="card"><b>Timing</b>Two 25-sample halves, five warmups each</div><div class="card"><b>Raw FP64</b>{raw_median:.6f} ms median</div></div>
...
<div class="finding">The directly timed genuine <code>N(0,1)</code> source has expected <code>X={genuine_expected_x:.3f}</code>, measured <code>X={genuine_actual_x:.3f}</code>, and takes {genuine_time:.6f} ms, or {genuine_time / raw_median:.3f}× raw FP64. Across the artificial hot-plus-uniform sweep, the slowest median is {sensitivity:.3f}× the fastest. {html.escape(crossing_text)}</div>
...
def main() -> None:
    ...
    print(f"raw FP64 median: {float(raw['median_ms']):.9f} ms")
    print(
