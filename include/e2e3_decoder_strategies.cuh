...
load_source_packet(const std::uint8_t *values) {
  ...
  if constexpr (Lanes == 1) {
    ...
    return *reinterpret_cast<const std::uint16_t *>(values);
  } else if constexpr (Lanes == 4) {
    return *reinterpret_cast<const std::uint32_t *>(values);
  } else {
    return static_cast<std::uint64_t>(
        *reinterpret_cast<const unsigned long long *>(values));
  }
}

template <int Lane>
__device__ __forceinline__ std::uint32_t raw_lane(std::uint64_t packet) {
  static_assert(Lane >= 0 && Lane < 8);
  return static_cast<std::uint32_t>((packet >> (8 * Lane)) & 0xffu);
}
...
decode_packet_impl(std::uint64_t packet, table_bundle tables,
...
template <typename Format, typename Strategy>
__device__ __forceinline__ decoded_packet<Strategy::lanes>
decode_packet(std::uint64_t packet, table_bundle tables) {
...
__global__ void gemv(const std::uint8_t *matrix, const std::uint8_t *vector,
  ...
  if constexpr (Strategy::pipelined) {
    ...
    if (pack < pack_count) {
      ...
      while (true) {
        const auto next_pack = pack + stride;
        std::uint64_t next_matrix{};
        std::uint64_t next_vector{};
        if (next_pack < pack_count) {
          next_matrix =
